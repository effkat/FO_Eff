...
class FlightReportPDF(FPDF):
    ...
    def __init__(self, route_name="Unknown Route", logo_path=None):
        ...
        self.add_font('Montserrat', '', 'Montserrat/static/Montserrat-Regular.ttf', uni=True) #change path
        self.add_font('Montserrat', 'B', 'Montserrat/static/Montserrat-Bold.ttf', uni=True)#change path
        self.add_font('Montserrat', 'I', 'Montserrat/static/Montserrat-Italic.ttf', uni=True)#change path
    ...
    if natures == []:
        ...
        for (alias, name), group in groupby(natures, key=lambda x: (x["alias"], x["feature_name"])):
            ...
            for i in range(0, len(group_list) - 1, 2):
                ...
                print (point_from['point_title'],point_to['point_title'])
...
def generate_docx_report(docx_path, output_docx_path):
    ...
    logo_run.add_picture("Beagle.png", width=Inches(1))

    
    
    
    document.save(output_docx_path2)
...
if __name__ == "__main__":
    ...
    generate_docx_report(output_docx_path, output_docx_path2)